...
void t0_init(void)
{
    TH0 = (65536 - 5000) / 256; // 表示計數 5000 步後觸發中斷
    TL0 = (65536 - 5000) % 256;
...
void Timer0_ISR(void) __interrupt(1) __using(1) // timer interrupt
{
    TH0 = (65536 - 5000) / 256;
    TL0 = (65536 - 5000) % 256;
    IT0 = 1;
    if (cnt >= 8) // 5000*8*1us = 0.04 second
    {
        cnt = 0;
    }
...
void External_ISR(void) __interrupt(0) __using(1) // exteral interrupt
{
    KeyScan();
    EX0 = 0;
}
